...
// type aaa = () as Future::Output;
// type WsFunc = std::sync::Arc<fn() -> dyn std::future::Future<Output = ()>>;
type WsFunc = Box<dyn Fn() -> std::pin::Pin<Box<dyn std::future::Future<Output = ()>>>>;

pub async fn init_ws_conn(data: web::Data<AppState>, ws: WebSocket, async_func: WsFunc) -> impl actix_web::Responder {
    // stream is the async iterator of incoming client websocket messages.
    // res is the response we return to client.
    // tx is a sender to push new websocket message to client response.
    let (mut stream, res, tx) = ws.into_parts();
